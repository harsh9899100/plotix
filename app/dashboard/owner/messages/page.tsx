...
const CONVERSATIONS = [
  { id:"c1", participantName:"Arjun Mehta", participantAvatar:"https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=60&h=60&fit=crop&crop=face", lastMessage:"I wanted to follow up on the 3BHK viewing.", time:new Date(Date.now()-30*60000), unread:1, messages:[
    { from:"buyer", text:"Hello, I wanted to follow up on the 3BHK viewing.", time:new Date(Date.now()-30*60000) },
    { from:"owner", text:"Hi Arjun! Yes, are you still interested?", time:new Date(Date.now()-25*60000) },
  ]},
  { id:"c2", participantName:"Meera Patel", participantAvatar:"", lastMessage:"When can I schedule a viewing?", time:new Date(Date.now()-2*3600000), unread:0, messages:[
    { from:"buyer", text:"When can I schedule a viewing?", time:new Date(Date.now()-2*3600000) },
  ]},
]
...
export default function OwnerMessagesPage() {
  ...
  const sendReply = () => {
    ...
    const msg = { from:"owner", text:reply, time:new Date() }
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {active.messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.from === "owner" ? "justify-end" : "justify-start"}`}>
                  <div className={msg.from === "owner" ? "chat-bubble-sent" : "chat-bubble-recv"}>
                    <p>{msg.text}</p>
                    <p className={`text-[10px] mt-1 ${msg.from === "owner" ? "text-white/50" : "text-stone-400"}`}>{formatTimeAgo(msg.time)}</p>
                  </div>
                </div>
              ))}
            </div>
